...
func main() {
	//bikin koneksi
	db, err = gorm.Open("mysql", "root:@/go_rest_api_crud?charset=uf8&parseTime=True")
	...
	db.AutoMigrate(&Product{}) //migrasi data product ke database
	handleRequests()
}
...
func createProduct(w http.ResponseWriter, r *http.Request) {
	payloads, _ := ioutil.ReadAll(r.Body) //nangkep payload
...
func getProduct(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r) //get product pake ID
	productID := vars["id"]
...
func updateProduct(w http.ResponseWriter, r *http.Request) {
	...
	//update datanya pake payload yang dikirim
